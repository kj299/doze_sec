...
echo try { >> "%PSRUN%"
echo   $remoteVer=(Invoke-WebRequest $remoteUrl -UseBasicParsing -TimeoutSec 10 -EA Stop).Content.Trim() >> "%PSRUN%"
...
echo   } >> "%PSRUN%"
echo } catch { >> "%PSRUN%"
echo   Write-Output ('  [INFO] Update check failed: '+$_.Exception.Message) >> "%PSRUN%"
echo } >> "%PSRUN%"
