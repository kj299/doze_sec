...
:: Outranks 2 and 4, and -- by the same more-actionable-signal rationale as
:: the 6-over-4 rule below -- also outranks the partial-audit code 6 (8 is
:: not LSS 5, so the escalation below leaves it alone). Never outranks the
:: fatal/abort codes 1/3/5/7.
...
echo   } >> "%PSRUN%"
echo   [void]$html.AppendLine('^<h3 style="color:#56cfe1;margin-top:16px;font-size:14px"^>To Apply (requires admin)^</h3^>') >> "%PSRUN%"
