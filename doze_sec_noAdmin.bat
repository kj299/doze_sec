...
echo   [void]$html.AppendLine('^<p^>This script reverts risky configurations detected above back to safe defaults. ^<strong^>Requires admin to apply. Review every command before running.^</strong^>^</p^>') >> "%PSRUN%"
...
echo   } >> "%PSRUN%"
echo   [void]$html.AppendLine('^<h3 style="color:#e94560;margin-top:16px;font-size:14px"^>To Apply (requires admin)^</h3^>') >> "%PSRUN%"
