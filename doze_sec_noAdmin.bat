...
echo       } else { >> "%PSRUN%"
echo         if ($null -eq $sig^) { $why = 'no-file' } elseif ($certIssue^) { $why = if ($sig.Status -eq 'Valid' -and $sig.SignerCertificate.Subject -match $trusted^) { 'trusted-but-' + $certIssue } else { $certIssue } } elseif ($sig.Status -eq 'Valid'^) { $why = 'unexpected-signer' } elseif ($sig.Status -eq 'NotSigned'^) { $why = 'unsigned' } else { $why = [string]$sig.Status } >> "%PSRUN%"
