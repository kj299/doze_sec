...
:: -----------------------------------------------------------------------------
:: ====================================================================
::  WIN11 SECURITY FORENSIC AUDIT  v7.3-noAdmin
...
echo.
echo    %C_GREEN%-noAdmin%C_RESET%     Run without administrator privileges. Checks that need
echo                 admin are marked [DEFERRED] in the report. Use this when
echo                 you cannot elevate -- you still get coverage of user-level
echo                 threats: processes, persistence, files, network, and the
echo                 full Section 18 CTI IOC sweep.
echo.
...
echo                 SAFE: never resolves attacker/C2 IOC domains. Off by default.
echo.
...
echo      %C_MAGENTA%(* = requires admin)%C_RESET%
echo.
echo    Security Audit:
...
echo      7.  Windows services audit (partial without admin)
echo      8.  Firewall configuration*
echo      9.  Windows Defender and AV status*
echo      10. SMB, RDP and remote access (partial without admin)
echo      11. PowerShell security (partial without admin)
echo      12. Credential and LSASS protection
echo      13. System hardening (partial without admin)
echo      14. Suspicious files and file system anomalies
echo      15. Installed software and driver audit
echo      16. Windows Event Log anomalies (partial without admin)
echo      17. Nation-state threat indicators (partial without admin)
echo      18. CTI-driven IOC sweep (SENTINEL-X threat intelligence)
echo      %C_MAGENTA%(* = fully requires admin)%C_RESET%
echo.
echo  %C_BOLD%SECTION 18 -- CTI IOC SWEEP:%C_RESET%
echo    Reads IOC files from ThreatLists/ and matches against the live
echo    system. Works fully without admin. Sub-checks:
...
echo    Non-admin:     %%USERPROFILE%%\SecurityAudit\SecurityReport_[timestamp].txt
echo    SMART data:    [output dir]\SmartData\
echo    Event exports: [output dir]\EventExports\
echo    IOC copies:    [output dir]\ThreatLists\
echo.
...
:: ====================================================================
:: Use user-profile directory in noAdmin mode to avoid C:\ write failures
if "%NO_ADMIN_MODE%"=="1" (
    set "OUTDIR=%USERPROFILE%\SecurityAudit"
) else (
...
:: %OUTDIR%\ThreatLists is the live copy -- INIT 10/14 writes freshness
:: headers + upstream fetches here, Section 18 reads here. The repo's
...
if %errorlevel% equ 0 (
    ...
    if "%NO_ADMIN_MODE%"=="0" (
        echo.
        echo  %C_RED%[FATAL]%C_RESET% Must be run as Administrator.
...
:: ---- Create report header (admin check done, report is safe to write) ----
...
echo.>> "%REPORT%"
if "%IS_ADMIN%"=="0" (
    echo  [INFO] NON-ADMIN MODE: Some sections will be deferred.>> "%REPORT%"
...
if "%NETWORK_AVAIL%"=="1" if "%VT_SELF_SKIP%"=="0" if exist "%USERPROFILE%\.vt_token" (
    ...
    if exist "%SCRIPT_DIR%tools\vt_self_check.ps1" (
        ...
        echo         tools folder ships alongside doze_sec_noAdmin.bat ^(both must>> "%REPORT%"
        echo         live in the same directory^), then re-run.>> "%REPORT%"
    )
    echo.>> "%REPORT%"
)
...
if "%IS_ADMIN%"=="0" goto :f8_noadmin
goto :f8_admincheck
:f8_noadmin
echo  [DEFERRED - ADMIN REQUIRED] bcdedit requires administrator privileges.>> "%REPORT%"
echo %C_GREEN%[INIT 11/14]%C_RESET% [DEFERRED] bcdedit requires admin.
...
if "%IS_ADMIN%"=="0" goto :srp_noadmin
goto :srp_admincheck
:srp_noadmin
echo  [DEFERRED - ADMIN REQUIRED] Checkpoint-Computer requires administrator privileges.>> "%REPORT%"
echo %C_GREEN%[INIT 12/14]%C_RESET% [DEFERRED] System Restore requires admin.
set /a DEFERRED_COUNT+=1
goto :srp_done
:srp_admincheck
...
(echo  Admin    : %IS_ADMIN%  [1=admin, 0=non-admin])>> "%REPORT%"
...
if "%IS_ADMIN%"=="0" echo  %C_MAGENTA%[NON-ADMIN MODE]%C_RESET% Some checks will be deferred.
...
if "%IS_ADMIN%"=="0" goto :sec7_noadmin
sc query type= all state= all>> "%REPORT%" 2>&1
goto :sec7_done
:sec7_noadmin
echo  [DEFERRED - ADMIN REQUIRED] Full service enumeration (sc query type=all) requires admin.>> "%REPORT%"
set /a DEFERRED_COUNT+=1
:sec7_done
echo.>> "%REPORT%"
...
rem Add-MpPreference APPENDS one rule. Set-MpPreference REPLACES the whole
rem array, so advising it would silently wipe every other ASR rule an admin
rem or MDM had configured.
echo   Write-Output '          Enable per rule: Add-MpPreference -AttackSurfaceReductionRules_Ids GUID -AttackSurfaceReductionRules_Actions Enabled' >> "%PSRUN%"
echo   Write-Output '          (Add-, not Set-: Set-MpPreference replaces the whole rule array.)' >> "%PSRUN%"
echo } else { >> "%PSRUN%"
...
goto :sec13_secboot_done
:sec13_secboot_noadmin
echo  [DEFERRED - ADMIN REQUIRED] Confirm-SecureBootUEFI requires admin.>> "%REPORT%"
set /a DEFERRED_COUNT+=1
:sec13_secboot_done
...
echo  [DEFERRED - ADMIN REQUIRED] Get-BitLockerVolume and manage-bde require admin.>> "%REPORT%"
set /a DEFERRED_COUNT+=1
:sec13_bitlocker_done
...
if "%IS_ADMIN%"=="0" goto :sec13_drvsig_noadmin
...
goto :sec13_drvsig_done
:sec13_drvsig_noadmin
...
goto :sec13_cbs_done
:sec13_cbs_noadmin
echo  [DEFERRED - ADMIN REQUIRED] CBS system-file integrity needs admin ^(reading %%WINDIR%%\Logs\CBS^).>> "%REPORT%"
set /a DEFERRED_COUNT+=1
:sec13_cbs_done
...
if "%IS_ADMIN%"=="0" goto :sec17_portproxy_noadmin
...
goto :sec17_portproxy_done
:sec17_portproxy_noadmin
echo  [DEFERRED - ADMIN REQUIRED] netsh portproxy requires admin.>> "%REPORT%"
set /a DEFERRED_COUNT+=1
:sec17_portproxy_done
...
goto :sec17_spray_done
:sec17_spray_noadmin
echo  [DEFERRED - ADMIN REQUIRED] Security event log (4625) requires admin.>> "%REPORT%"
:sec17_spray_done
...
if "%IS_ADMIN%"=="0" goto :sec17_mpdet_noadmin
...
goto :sec17_mpdet_done
:sec17_mpdet_noadmin
echo Write-Output '[DEFERRED - ADMIN REQUIRED] Get-MpThreatDetection requires admin.' >> "%PSRUN%"
:sec17_mpdet_done
echo Write-Output '' >> "%PSRUN%"
...
:: Runtime path is primary (freshest INIT 10/14 fetches); repo path is
:: fallback if runtime is somehow missing. (closes #106)
...
rem Staged remediation: stage 1 applies the safe, reversible fixes; the
rem enforce script applies the changes worth observing in audit mode first;
rem the undo script reverses what either of them applied.
set "REMEDIATION_ENF=%OUTDIR%\Remediation_!TIMESTAMP!_enforce.ps1"
set "REMEDIATION_UNDO=%OUTDIR%\Remediation_!TIMESTAMP!_undo.ps1"

:: ---- Seed remediation script with a safety header -------------------
:: The script self-aborts unless the user edits the IReadAndUnderstand
:: flag. This prevents accidental remediation execution.
call :dz_seed_rem "%REMEDIATION%" "APPLY (stage 1) -- safe, reversible fixes" "There is NO automatic undo built in, but every change below carries an UNDO line, and Remediation_..._undo.ps1 collects them."
call :dz_seed_rem "%REMEDIATION_ENF%" "ENFORCE (stage 2) -- changes to observe first" "Run stage 1 first, leave it for a few days, then review what audit mode logged before running this."
call :dz_seed_rem "%REMEDIATION_UNDO%" "UNDO -- reverse what the other two applied" "Run this only to back out a change that broke something. Security improvements are intentionally NOT reversed here."
...
echo }else{ >> "%PSRUN%"
echo ck 'INFO' 'Security event log analysis deferred (requires admin)' >> "%PSRUN%"
echo ck 'INFO' 'netsh portproxy check deferred (requires admin)' >> "%PSRUN%"
echo } >> "%PSRUN%"
...
:: ===== DEFERRED CHECKS (non-admin only) ==============================
echo if($isAdmin -eq '0'){ >> "%PSRUN%"
echo sec 'DEFERRED CHECKS  (Re-run as Administrator for full audit)' >> "%PSRUN%"
echo ck 'INFO' 'Security Event Log analysis deferred' >> "%PSRUN%"
echo ck 'INFO' 'Firewall configuration check deferred' >> "%PSRUN%"
echo ck 'INFO' 'Windows Defender status check deferred' >> "%PSRUN%"
echo ck 'INFO' 'BitLocker and Secure Boot verification deferred' >> "%PSRUN%"
echo ck 'INFO' 'SMB server configuration check deferred' >> "%PSRUN%"
echo ck 'INFO' 'netsh portproxy tunnel check deferred' >> "%PSRUN%"
echo ck 'INFO' 'bcdedit driver signature check deferred' >> "%PSRUN%"
echo ck 'INFO' 'F8 boot menu and System Restore Point deferred' >> "%PSRUN%"
echo } >> "%PSRUN%"
echo. >> "%PSRUN%"
...
echo if($isAdmin -eq '0'){'##  MODE: NON-ADMIN (partial audit)'} >> "%PSRUN%"
echo '##' >> "%PSRUN%"
echo ('##  ' + $status) >> "%PSRUN%"
echo '##' >> "%PSRUN%"
echo $bar >> "%PSRUN%"
...
echo if ($isAdmin -eq '0') { >> "%PSRUN%"
echo '' >> "%PSRUN%"
echo '  NOTE: This was a NON-ADMIN audit. Some checks were deferred.' >> "%PSRUN%"
echo '  Re-run this script as Administrator for a complete audit.' >> "%PSRUN%"
echo } >> "%PSRUN%"
echo $bar >> "%PSRUN%"
echo '' >> "%PSRUN%"
...
rem addfix/addenforce take an UNDO command. A fix with no safe reversal must
rem pass '''' and the generated line says so -- the gap is stated, never silent.
rem '# FIX: ' is a machine-readable marker: the counter and report_html both key
rem on it, so a fix can never be silently uncounted because its verb is missing
rem from a hand-maintained prefix list (that bug shipped once already).
echo function remwrite($f,$tag,$cmd,$undo){ Add-Content -LiteralPath $f -Value ('# FIX: '+$tag); Add-Content -LiteralPath $f -Value $cmd; if($undo){ Add-Content -LiteralPath $f -Value ('# UNDO: '+$undo) } else { Add-Content -LiteralPath $f -Value '# UNDO: none -- this is a security improvement; reverse it manually only if it breaks something' }; Add-Content -LiteralPath $f -Value ''; if($undo){ Add-Content -LiteralPath $und -Value ('# reverses: '+$tag); Add-Content -LiteralPath $und -Value $undo; Add-Content -LiteralPath $und -Value '' } } >> "%PSRUN%"
echo function addfix($tag,$cmd,$undo){ remwrite $rem $tag $cmd $undo } >> "%PSRUN%"
echo function addenforce($tag,$cmd,$undo){ remwrite $enf $tag $cmd $undo } >> "%PSRUN%"
...
echo if($joined -match 'WinRM RUNNING'){ addfix 'Stop and disable WinRM' "Stop-Service WinRM -Force; Set-Service WinRM -StartupType Disabled" 'Set-Service WinRM -StartupType Automatic; Start-Service WinRM' } >> "%PSRUN%"
...
rem the whole array and silently wipe every other ASR rule an admin or MDM set.
rem Stage 1 is AuditMode only, and only for rules not already in Block, so a
rem rule already enforcing is never downgraded.
echo if(led 'WARNING' '9' 'T1562.001' 'ASR rules absent or not in Block mode'){ addfix 'Put the key Defender ASR rules into AUDIT mode (logs what would be blocked, blocks nothing; records prior state to ProgramData\doze_sec_asr_prior.txt)' '$k=@(''9e6c4e1f-7d60-472f-ba1a-a39ef669e4b2'',''56a863a9-875e-4185-98a7-b882c64b5ce5'',''3b576869-a4ec-4529-8536-b80a7769e899'',''92e97fa1-2edf-4476-bdd6-9dd0b4dddc7b'',''c1db55ab-c21a-4637-bb3f-a12568109d35''^); $p=Get-MpPreference; $cur=@{}; for($i=0;$i -lt @($p.AttackSurfaceReductionRules_Ids^).Count;$i++^){ $cur[([string]$p.AttackSurfaceReductionRules_Ids[$i]^).ToLowerInvariant(^)]=[int]$p.AttackSurfaceReductionRules_Actions[$i] }; $todo=@($k ^| Where-Object { $cur[$_] -ne 1 }^); if($todo^){ $sf=Join-Path $env:ProgramData ''doze_sec_asr_prior.txt''; if(-not (Test-Path -LiteralPath $sf^)^){ Set-Content -LiteralPath $sf -Value @($todo ^| ForEach-Object { $_+''=''+[int]$cur[$_] }^) }; foreach($g in $todo^){ Add-MpPreference -AttackSurfaceReductionRules_Ids $g -AttackSurfaceReductionRules_Actions AuditMode } } else { Write-Host ''All five key ASR rules are already in Block mode; nothing to do.'' }' '$sf=Join-Path $env:ProgramData ''doze_sec_asr_prior.txt''; if(Test-Path -LiteralPath $sf^){ $map=@{''0''=''Disabled'';''1''=''Enabled'';''2''=''AuditMode'';''6''=''Warn''}; foreach($ln in (Get-Content -LiteralPath $sf^)^){ $parts=$ln.Split(''=''^); $act=$map[$parts[1]]; if(-not $act^){ $act=''Disabled'' }; Add-MpPreference -AttackSurfaceReductionRules_Ids $parts[0] -AttackSurfaceReductionRules_Actions $act }; Write-Host ''ASR rules restored to the state recorded in ProgramData\doze_sec_asr_prior.txt.'' } else { Write-Host ''No doze_sec_asr_prior.txt in ProgramData, so nothing was recorded to restore. Check the current state with (Get-MpPreference^).AttackSurfaceReductionRules_Actions'' }' } >> "%PSRUN%"
echo if(led 'WARNING' '9' 'T1562.001' 'ASR rules absent or not in Block mode'){ addenforce 'Put the key Defender ASR rules into BLOCK mode -- review what audit mode logged first; may break Office add-ins and macro workflows' '$k=@(''9e6c4e1f-7d60-472f-ba1a-a39ef669e4b2'',''56a863a9-875e-4185-98a7-b882c64b5ce5'',''3b576869-a4ec-4529-8536-b80a7769e899'',''92e97fa1-2edf-4476-bdd6-9dd0b4dddc7b'',''c1db55ab-c21a-4637-bb3f-a12568109d35''^); $p=Get-MpPreference; $cur=@{}; for($i=0;$i -lt @($p.AttackSurfaceReductionRules_Ids^).Count;$i++^){ $cur[([string]$p.AttackSurfaceReductionRules_Ids[$i]^).ToLowerInvariant(^)]=[int]$p.AttackSurfaceReductionRules_Actions[$i] }; $todo=@($k ^| Where-Object { $cur[$_] -ne 1 }^); if($todo^){ $sf=Join-Path $env:ProgramData ''doze_sec_asr_prior.txt''; if(-not (Test-Path -LiteralPath $sf^)^){ Set-Content -LiteralPath $sf -Value @($todo ^| ForEach-Object { $_+''=''+[int]$cur[$_] }^) }; foreach($g in $todo^){ Add-MpPreference -AttackSurfaceReductionRules_Ids $g -AttackSurfaceReductionRules_Actions Enabled } } else { Write-Host ''All five key ASR rules are already in Block mode; nothing to do.'' }' '$sf=Join-Path $env:ProgramData ''doze_sec_asr_prior.txt''; if(Test-Path -LiteralPath $sf^){ $map=@{''0''=''Disabled'';''1''=''Enabled'';''2''=''AuditMode'';''6''=''Warn''}; foreach($ln in (Get-Content -LiteralPath $sf^)^){ $parts=$ln.Split(''=''^); $act=$map[$parts[1]]; if(-not $act^){ $act=''Disabled'' }; Add-MpPreference -AttackSurfaceReductionRules_Ids $parts[0] -AttackSurfaceReductionRules_Actions $act }; Write-Host ''ASR rules restored to the state recorded in ProgramData\doze_sec_asr_prior.txt.'' } else { Write-Host ''No doze_sec_asr_prior.txt in ProgramData, so nothing was recorded to restore. Check the current state with (Get-MpPreference^).AttackSurfaceReductionRules_Actions'' }' } >> "%PSRUN%"
...
echo Add-Content -LiteralPath $rem -Value 'Write-Host ''*** All queued commands applied. Some settings need a reboot. Each change above carries an UNDO line, and the _undo.ps1 beside this file collects them. ***''; exit 0' >> "%PSRUN%"
rem Count the '# FIX: ' markers, not command verbs. The old verb list gave the
rem count a floor of 2 (the header and footer Write-Host lines matched it) and
rem disagreed with report_html, which required a preceding comment.
echo $fixCount = @(Get-Content -LiteralPath $rem ^| Where-Object { $_ -like '# FIX: *' }^).Count >> "%PSRUN%"
echo $enfCount = @(Get-Content -LiteralPath $enf -EA SilentlyContinue ^| Where-Object { $_ -like '# FIX: *' }^).Count >> "%PSRUN%"
echo if($fixCount -eq 0){ Add-Content -LiteralPath $rem -Value '# No auto-fixable findings detected. The system is either clean or the findings require manual remediation (see the report).' } >> "%PSRUN%"
echo Write-Output '' >> "%PSRUN%"
echo Write-Output '######################################################################' >> "%PSRUN%"
...
rem Stage 2 and the undo script are only advertised when they hold something,
rem so a clean machine is not told to go and run two empty files.
echo if($enfCount -gt 0){ Write-Output ('##  Stage 2  : ' + $enf) ; Write-Output ('##             ' + $enfCount + ' change(s) to apply only AFTER reviewing what audit mode logged') } >> "%PSRUN%"
echo if(Test-Path -LiteralPath $und){ if(@(Get-Content -LiteralPath $und ^| Where-Object { $_ -like '# reverses:*' }^).Count -gt 0){ Write-Output ('##  Undo     : ' + $und) } } >> "%PSRUN%"
echo Write-Output '##' >> "%PSRUN%"
...
echo.
echo %C_BOLD%====================================================================%C_RESET%
if "%EXIT_CODE%"=="0" echo  %C_GREEN%RESULT: No issues found in the checks that ran.%C_RESET% Not a safety guarantee -- see COVERAGE ^& CONFIDENCE and READ THIS FIRST in the report.
if "%EXIT_CODE%"=="2" echo  %C_YELLOW%RESULT: Issues found.%C_RESET% See SUMMARY at end of report.
if "%EXIT_CODE%"=="6" echo  %C_MAGENTA%RESULT: Partial audit (non-admin).%C_RESET% !DEFERRED_COUNT! check(s) deferred.
if "%EXIT_CODE%"=="8" echo  %C_RED%RESULT: CRITICAL findings. Treat as incident response.%C_RESET% See SUMMARY.
...
:: ====================================================================
:: ====================================================================
:: :dz_seed_rem -- write the shared safety header of a generated script.
:: Args: %1=target path  %2=one-line purpose  %3=extra guidance line
:: The three generated scripts (apply / enforce / undo) share this header
:: so the elevation assert and the colour-free guard cannot drift apart.
:: Placed after the final exit so it is only ever entered via `call`.
:: ====================================================================
:dz_seed_rem
echo # ============================================================== > "%~1"
echo # %~2 -- generated by doze_sec v%SCRIPT_VERSION% >> "%~1"
echo # Host: %COMPUTERNAME%    Generated: %date% %time% >> "%~1"
echo # ============================================================== >> "%~1"
echo # REVIEW EVERY LINE BEFORE RUNNING. Some commands reboot or >> "%~1"
echo # change security-critical settings. Run as ADMIN, from a >> "%~1"
echo # trusted PowerShell session. >> "%~1"
echo # >> "%~1"
echo # %~3 >> "%~1"
echo # >> "%~1"
echo # To enable execution, change the next line to $true. Change nothing else. >> "%~1"
echo # ============================================================== >> "%~1"
echo $IReadAndUnderstand=$false >> "%~1"
...
echo if(-not $IReadAndUnderstand){ Write-Host '*** ABORT -- nothing was applied. ***'; Write-Host '*** Open this file, review every command, set $IReadAndUnderstand=$true, then re-run. ***'; exit 1 } >> "%~1"
echo $__id=[Security.Principal.WindowsIdentity]::GetCurrent() >> "%~1"
...
echo $ErrorActionPreference='Continue'; $__errBefore=$Error.Count >> "%~1"
echo Write-Host '*** %~2 -- press Ctrl+C now to abort. ***'; Start-Sleep -Seconds 3 >> "%~1"
echo. >> "%~1"
goto :eof
