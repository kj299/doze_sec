...
:: Load GitHub PAT (same scheme as self-update above)
echo $token = $env:DOZESEC_TOKEN >> "%PSRUN%"
echo if(-not $token){ $tf = Join-Path $env:USERPROFILE '.dozesec_token'; if(Test-Path -LiteralPath $tf){ $token = (Get-Content -LiteralPath $tf -Raw -EA SilentlyContinue).Trim() } } >> "%PSRUN%"
echo $headers = @{ 'User-Agent' = 'doze_sec' } >> "%PSRUN%"
echo if($token){ $headers['Authorization'] = "Bearer $token" } >> "%PSRUN%"
echo try { >> "%PSRUN%"
echo   Invoke-WebRequest $listUrl -OutFile $dest -UseBasicParsing -TimeoutSec 15 -Headers $headers -EA Stop >> "%PSRUN%"
echo   Write-Output ('  [OK] Threat list updated: '+$dest) >> "%PSRUN%"
echo } catch { >> "%PSRUN%"
echo   $sc=$null; try{ $sc=[int]$_.Exception.Response.StatusCode }catch{} >> "%PSRUN%"
echo   if($sc -eq 404 -and -not $token){ Write-Output '  [INFO] Threat list not available (repo private, no PAT). See README for DOZESEC_TOKEN setup.' } >> "%PSRUN%"
echo   else{ Write-Output ('  [INFO] Threat list not available at configured URL.') } >> "%PSRUN%"
echo } >> "%PSRUN%"
"%PWSH%" -NoProfile -ExecutionPolicy Bypass -File "%PSRUN%">> "%REPORT%" 2>&1
:update_done
echo.>> "%REPORT%"
...
:: ====================================================================
echo ====================================================================>> "%REPORT%"
echo  [18/18] CTI-DRIVEN THREAT INDICATOR SWEEP>> "%REPORT%"
echo  Source: SENTINEL-X Cyber Threat Intelligence Skill v3.0>> "%REPORT%"
...
(
    echo # ==============================================================
    echo # Remediation script generated by doze_sec v%SCRIPT_VERSION%
    echo # Host: %COMPUTERNAME%    Generated: %date% %time%
    echo # ==============================================================
    echo # REVIEW EVERY LINE BEFORE RUNNING. Some commands reboot or
    echo # change security-critical settings. Run as ADMIN, from a
    echo # trusted PowerShell session.
    echo #
    echo # To enable execution, change $IReadAndUnderstand=$false to $true
    echo # ==============================================================
    echo $IReadAndUnderstand=$false
    echo if(-not $IReadAndUnderstand){ Write-Host '[ABORT] Open Remediation script, review commands, flip $IReadAndUnderstand=$true, then re-run.' -Fore Yellow; exit 1 }
    echo Write-Host 'Applying remediation commands. Press Ctrl+C now to abort.' -Fore Cyan; Start-Sleep -Seconds 3
    echo.
) > "%REMEDIATION%"
...
:: Scans $r and appends PS commands to $rem for every finding with a
:: safe, reversible auto-fix. Manual-only findings (Winlogon tamper,
:: IFEO hijack, ransomware IOCs) are intentionally excluded.
...
echo if($joined -match 'WMI EventFilter subscriptions present'){ addfix 'Remove WMI permanent EventFilter subscriptions' "Get-CimInstance -Namespace root\subscription -ClassName __EventFilter -EA SilentlyContinue ^| Remove-CimInstance; Get-CimInstance -Namespace root\subscription -ClassName __EventConsumer -EA SilentlyContinue ^| Remove-CimInstance; Get-CimInstance -Namespace root\subscription -ClassName __FilterToConsumerBinding -EA SilentlyContinue ^| Remove-CimInstance" } >> "%PSRUN%"
...
echo.
echo %C_BOLD%====================================================================%C_RESET%
if "%EXIT_CODE%"=="0" echo  %C_GREEN%RESULT: All checks passed.%C_RESET%
...
echo  0=Success  1=Error  2=Warning  3=UnsupportedOS  4=RebootPending  5=RanFromTEMP  6=PartialNoAdmin>> "%REPORT%"
...
:: Delete RunOnce key on clean exit (0=success, 2=warning, 6=partial all count as "completed")
